package com.guance.exporter.guance.metric;
...
import static com.guance.exporter.guance.http.OKHTTPClient.METRIC_CATEGORY;

public class GuanceMetricExporter implements MetricExporter {
    private static final Logger internalLogger =
            Logger.getLogger(GuanceMetricExporter.class.getName());
    private final ThrottlingLogger logger = new ThrottlingLogger(internalLogger);
    public static final String METRIC_NAME = "otel-service";
    private final OKHTTPClient delegate;

    public GuanceMetricExporter() {
        logger.log(Level.INFO, "init GuanceMetricExporter");
        delegate = new OKHTTPClient();
    }

    public GuanceMetricExporter setEndpoint( String endpoint) {
        delegate.setEndpoint(endpoint);
        return this;
    }

    public GuanceMetricExporter setToken( String token) {
        if (token.equals("")) {
            logger.log(
    ...
    public String toPointDB(
        ...
        Point.Builder pointBuilder =
                Point.measurement(METRIC_NAME).addField(name, value).tag("host", GuanceUtils.getHostName());
